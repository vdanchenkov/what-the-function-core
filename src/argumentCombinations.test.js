...
describe('argumentCombinations()', () => {
  it('returns array of argument combination definitions', () => {
    ...
    expect(result).to.include({ args: ['a', 'c', 'b'], argsOrder: [0, 2, 1], argsLabels: ["'a'", "'c'", "'b'"] });
    expect(result).to.include({ args: ['c', 'a', 'b'], argsOrder: [2, 0, 1], argsLabels: ["'c'", "'a'", "'b'"] });
  });

  it('skips permutations for arguments after 4th', () => {
    const result = argumentCombinations('a', 'b', 'c', 'd');
    expect(result.length).to.be.eq(6);
    expect(result).to.include({ args: ['a', 'c', 'b', 'd'], argsOrder: [0, 2, 1, 3], argsLabels: ["'a'", "'c'", "'b'", "'d'"] });
    expect(result).to.include({ args: ['c', 'a', 'b', 'd'], argsOrder: [2, 0, 1, 3], argsLabels: ["'c'", "'a'", "'b'", "'d'"] });
  })
})